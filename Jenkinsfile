pipeline {
    ...
    environment {
      ...
      stages {
        ...
        stage('Generate Target Platform') {
            steps {        
                script { 
                    ...
                    sh 'env'
                    sh 'mvn clean verify -f releng/org.eclipse.capella.simulink.target/pom.xml'
                   }         
             }
        }
        ...
        stage('Build and Package') {
              steps {
                  script {
                      ...
                      sh "mvn -DskipTests=true -Djacoco.skip=true -DjavaDocPhase=none ${customParams} clean package -f pom.xml"
                   }         
             }
        }
        ...
        stage('Deploy to Nightly') {
              steps {
                script {
                    ...
                    deployer.addonNightlyDropins("${WORKSPACE}/releng/org.eclipse.capella.simulink.updatesite/target/*-dropins.zip", deploymentDirName)
        ...
        //             eclipse.installFeature("${CAPELLA_PRODUCT_PATH}", "file:/${WORKSPACE}/releng/org.eclipse.capella.simulink.updatesite/target/repository/".replace("\\", "/"), 'org.eclipse.capella.simulink.core.feature.feature.group')
        //             eclipse.installFeature("${CAPELLA_PRODUCT_PATH}", "file:/${WORKSPACE}/releng/org.eclipse.capella.simulink.updatesite/target/repository/".replace("\\", "/"), 'org.eclipse.capella.simulink.capella.feature.feature.group')
        //            }         
        //      }
        // }
